...
//解锁GD32的FLASH
void GDFLASH_Unlock(void)
{
	FLASH->KEYR=FLASH_KEY1;	//写入解锁序列.
	FLASH->KEYR=FLASH_KEY2; 
}
//flash上锁
void GDFLASH_Lock(void)
{
...
//2,操作异常
//0XFF,超时       
u8 GDFLASH_WaitDone(u32 time)
{
	u8 res;
	do
	{
		res=GDFLASH_GetStatus();
...
{
	u8 res=0;
	res=GDFLASH_WaitDone(200000);//等待上次操作结束,最大2s    
	if(res==0)
	{ 
		...
		FLASH->CR|=1<<1;	//扇区擦除 
		FLASH->CR|=1<<16;	//开始擦除		  
		res=GDFLASH_WaitDone(200000);//等待操作结束,最大2s  
		if(res!=1)			//非忙
		{
...
//返回值:0,写入成功
//    其他,写入失败
u8 GDFLASH_WriteWord(u32 faddr, u32 dat)
{
	u8 res;	   	    
	res=GDFLASH_WaitDone(0XFF);	 
	if(res==0)//OK
	{
		FLASH->CR&=~(3<<8);	//清除PSIZE原来的设置
		FLASH->CR|=2<<8;	//设置为32bit宽,确保VCC=2.7~3.6V之间!!
 		FLASH->CR|=1<<0;	//编程使能
		*(vu32*)faddr=dat;	//写入数据
		res=GDFLASH_WaitDone(0XFF);//等待操作完成,一个字编程,最多100us.
		if(res!=1)//操作成功
		{
			FLASH->CR&=~(1<<0);//清除PG位.
		}
...
//addr:flash地址
//返回值:0~11,即addr所在的扇区
u8 GDFLASH_GetFlashSector(u32 addr)
{
...
//从指定地址开始写入指定长度的数据
//特别注意:因为GD32F4的扇区实在太大,没办法本地保存扇区数据,所以本函数
...
void GDFLASH_Write(u32 WriteAddr,u32 *pBuffer,u32 NumToWrite)	
{ 
	u8 status=0;
	u32 addrx=0;
	u32 endaddr=0;	
  	if(WriteAddr<GD32_FLASH_BASE||WriteAddr%4)return;	//非法地址
	GDFLASH_Unlock();									//解锁 
	...
	if(addrx<0X1FFF0000)			//只有主存储区,才需要执行擦除操作!!
	{
		while(addrx<endaddr)		//扫清一切障碍.(对非FFFFFFFF的地方,先擦除)
		{
			...
			{   
				status=GDFLASH_EraseSector(GDFLASH_GetFlashSector(addrx));
	...
	if(status==0)
	{
		...
		{
			if(GDFLASH_WriteWord(WriteAddr,*pBuffer))//写入数据
			{ 
				break;	//写入异常
			}
			WriteAddr+=4;
