...
{ 
	...
	RCU->AHB1EN|=1<<22;		//DMA1时钟使能 
 
	DMA_DeInit(DMA1_Stream1);//等待DMA2_Stream1
  ...
  {
		DMA_DoubleBufferModeCmd(DMA1_Stream1,ENABLE);//双缓冲模式
	  DMA_MemoryTargetConfig(DMA1_Stream1,DMA_Memory1BaseAddr,DMA_Memory_1);//配置目标地址1
		DMA_ITConfig(DMA1_Stream1,DMA_IT_TC,ENABLE);//开启传输完成中断
...
{        
	if(DMA_GetFlagStatus(DMA1_Stream1,DMA_FLAG_TCIF1)==SET)//DMA2_Steam1,传输完成标志
	{  
		 DMA_ClearFlag(DMA1_Stream1,DMA_FLAG_TCIF1);//清除传输完成中断
...
void My_DCMI_Init(void)
{
	...
	RCU->AHB1EN|=((u32)(1<<0|1<<1|1<<2|1<<4));
	RCU->AHB2EN|=1<<0; // DCMI时钟使能 
  ...
  GPIO_Init(GPIOE, &GPIO_InitStructure);//初始化	
	...
	GPIO_PinAFConfig(GPIOA,GPIO_PinSource4,GPIO_AF_13); //PA4,AF13  DCMI_HSYNC
	GPIO_PinAFConfig(GPIOA,GPIO_PinSource6,GPIO_AF_13); //PA6,AF13  DCMI_PCLK  
 	GPIO_PinAFConfig(GPIOB,GPIO_PinSource7,GPIO_AF_13); //PB7,AF13  DCMI_VSYNC 
 	GPIO_PinAFConfig(GPIOC,GPIO_PinSource6,GPIO_AF_13); //PC6,AF13  DCMI_D0  
 	GPIO_PinAFConfig(GPIOC,GPIO_PinSource7,GPIO_AF_13); //PC7,AF13  DCMI_D1 
	GPIO_PinAFConfig(GPIOC,GPIO_PinSource8,GPIO_AF_13); //PC8,AF13  DCMI_D2
	GPIO_PinAFConfig(GPIOC,GPIO_PinSource9,GPIO_AF_13); //PC9,AF13  DCMI_D3
	GPIO_PinAFConfig(GPIOC,GPIO_PinSource11,GPIO_AF_13);//PC11,AF13 DCMI_D4 
	GPIO_PinAFConfig(GPIOB,GPIO_PinSource6,GPIO_AF_13); //PB6,AF13  DCMI_D5 
	GPIO_PinAFConfig(GPIOE,GPIO_PinSource5,GPIO_AF_13); //PE5,AF13  DCMI_D6
	GPIO_PinAFConfig(GPIOE,GPIO_PinSource6,GPIO_AF_13); //PE6,AF13  DCMI_D7
...
{  
 // LCD_SetCursor(0,0);  
	...
	DMA_Cmd(DMA1_Stream1, ENABLE);//开启DMA2,Stream1 
...
{ 
  DCMI_CaptureCmd(DISABLE);//DCMI捕获使关闭	
	...
	DMA_Cmd(DMA1_Stream1,DISABLE);//关闭DMA2,Stream1
} 
////////////////////////////////////////////////////////////////////////////////
