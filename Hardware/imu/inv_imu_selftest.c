                         ...
                         inv_imu_selftest_output_t *st_output)
{
	...
	st_output->gyro_status |= ((data & ST_STATUS2_ST_INCOMPLETE_MASK) >> ST_STATUS2_ST_INCOMPLETE_POS) << 1;
	...
	st_output->accel_status = (data & ST_STATUS1_ACCEL_ST_PASS_MASK) >> ST_STATUS1_ACCEL_ST_PASS_POS;
                                         ...
                                         const inv_imu_selftest_parameters_t st_params)
{
	...
	value &= ~((uint8_t)ST_CONFIG_ST_NUMBER_SAMPLE_MASK | (uint8_t)ST_CONFIG_ACCEL_ST_LIM_MASK |
	           (uint8_t)ST_CONFIG_GYRO_ST_LIM_MASK);
	value |= (uint8_t)st_params.st_num_samples | (uint8_t)ST_CONFIG_ACCEL_ST_LIM_50 |
