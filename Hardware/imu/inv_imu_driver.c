...
int inv_imu_device_reset(struct inv_imu_device *s)
{
	...
	data = 0x08; 
	status |= inv_imu_write_reg(s, APEX_CONFIG0, 1, &data);
	data = 0x02; 
	status |= inv_imu_write_reg(s, APEX_CONFIG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, WOM_CONFIG, 1, &data);
	data = 0x01; 
	status |= inv_imu_write_reg(s, FIFO_CONFIG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, FIFO_CONFIG2, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, FIFO_CONFIG3, 1, &data);
	data = 0x20; 
	status |= inv_imu_write_reg(s, FIFO_CONFIG5_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, ST_CONFIG_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, INT_SOURCE7_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, INT_SOURCE8_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, INT_SOURCE9_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, INT_SOURCE10_MREG1, 1, &data);
	data = 0xA2; 
	status |= inv_imu_write_reg(s, APEX_CONFIG2_MREG1, 1, &data);
	data = 0x85; 
	status |= inv_imu_write_reg(s, APEX_CONFIG3_MREG1, 1, &data);
	data = 0x51; 
	status |= inv_imu_write_reg(s, APEX_CONFIG4_MREG1, 1, &data);
	data = 0x80; 
	status |= inv_imu_write_reg(s, APEX_CONFIG5_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, APEX_CONFIG9_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, APEX_CONFIG10_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, APEX_CONFIG11_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, ACCEL_WOM_X_THR_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, ACCEL_WOM_Y_THR_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, ACCEL_WOM_Z_THR_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, OFFSET_USER0_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, OFFSET_USER1_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, OFFSET_USER2_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, OFFSET_USER3_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, OFFSET_USER4_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, OFFSET_USER5_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, OFFSET_USER6_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, OFFSET_USER7_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, OFFSET_USER8_MREG1, 1, &data);
	data = 0x00; 
	status |= inv_imu_write_reg(s, APEX_CONFIG12_MREG1, 1, &data);
...
static int reload_otp(struct inv_imu_device *s)
{
	 ...
	 */
	status |= inv_imu_read_reg(s, OTP_CONFIG_MREG1, 1, &data);
	data &= ~OTP_CONFIG_OTP_COPY_MODE_MASK;
	data |= OTP_CONFIG_OTP_COPY_TRIM;
	status |= inv_imu_write_reg(s, OTP_CONFIG_MREG1, 1, &data);

	/* 
	 * Set otp_power_down register field
	 */
	 ...
	 */
	status |= inv_imu_read_reg(s, OTP_CTRL7_MREG2, 1, &data);
	data &= ~OTP_CTRL7_OTP_RELOAD_MASK;
	data |= OTP_CTRL7_OTP_RELOAD_EN;
	status |= inv_imu_write_reg(s, OTP_CTRL7_MREG2, 1, &data);
...
//			s->accel_start_time_us = inv_imu_get_time_us();
	...
	//add register print//
	 status |= inv_imu_read_reg(s, ACCEL_CONFIG0, 1, &value);
	 INV_MSG(INV_MSG_LEVEL_INFO, "ACCEL_CONFIG0 = 0x%x",value);
	 status |= inv_imu_read_reg(s, PWR_MGMT0, 1, &value);
	 INV_MSG(INV_MSG_LEVEL_INFO, "PWR_MGMT0 = 0x%x",value);
	 status |= inv_imu_read_reg(s, MCLK_RDY, 1, &value);
     INV_MSG(INV_MSG_LEVEL_INFO, "MCLK_RDY = 0x%x",value);
	return status;
}
...
static int configure_serial_interface(struct inv_imu_device *s)
{
	...
	case UI_SPI4:
		...
		value = 1 << DEVICE_CONFIG_SPI_AP_4WIRE_POS;
